...
## Split-Source Feasibility Audit

Scope: only providers and artifacts already used by this repository were inspected. No paid provider was added, and no unrelated website scraping is recommended.

| Metric | Available without new paid source? | Source/provider | Endpoint or existing response | Numerator | Denominator | Sample-size support | Identifiers required | Additional API calls | Rate-limit impact | Paid-tier implications | Recommendation |
|---|---:|---|---|---|---|---|---|---:|---|---|---|
| Pitcher home K% | Not currently | MLB StatsAPI game logs already used by workload fetch | Existing `fetchPitcherWorkloadData` appearances can derive if home/away is retained per appearance | strikeouts in home starts/appearances | batters faced in home starts/appearances | Yes if per-appearance BF and venue/home flag are retained | pitcherId, team/game home-away | Low; can reuse game log payload if enriched | Low | None | Derive in workload layer later; leave null until tested. |
| Pitcher away K% | Not currently | MLB StatsAPI game logs already used by workload fetch | Same as above | strikeouts away | batters faced away | Yes | pitcherId, team/game home-away | Low | Low | None | Derive together with home K%; leave null now. |
| Pitcher home whiff% | No | Current Statcast-style pitcher raw fields | Current raw has season `whiffRate`, no game/home split | unavailable | unavailable | No | pitcherId | Unknown if using current raw source | Unknown | Unknown | Leave null; do not infer from season whiff. |
| Pitcher away whiff% | No | Current Statcast-style pitcher raw fields | Same as above | unavailable | unavailable | No | pitcherId | Unknown | Unknown | Unknown | Leave null. |
| Pitcher vs-LHB K% | No reliable current feed | MLB StatsAPI game logs do not expose batter-handed K split in current pipeline | none currently retained | strikeouts vs LHB | BF vs LHB | No current support | pitcherId plus batter handedness events | Would require new split/event fetch design | Medium/high | None if MLB API supports required split, unproven here | Leave null until a verified StatsAPI split endpoint is added and tested. |
| Pitcher vs-RHB K% | No reliable current feed | Same as above | none currently retained | strikeouts vs RHB | BF vs RHB | No current support | pitcherId plus batter handedness events | Medium/high | Medium/high | None if supported, unproven | Leave null. |
| Opponent home K% | Derivable, not currently | MLB StatsAPI team hitting stats already used | Current `fetchTeamStats` season/recent team hitting; no home split requested | team strikeouts at home | team plate appearances at home | Yes if split endpoint/response exposes PA | teamId, season | Additional per-team split calls or expanded stat call | Medium for slate teams | None | Feasible enrichment candidate; leave null until implemented with validation. |
| Opponent away K% | Derivable, not currently | MLB StatsAPI team hitting stats already used | Same as above | team strikeouts away | team PA away | Yes | teamId, season | Medium | Medium | None | Feasible with same implementation as home K%. |
| Opponent home whiff% | No | Current team hitting stats and lineup proxies | Current StatsAPI team context has no whiff; raw lineup has hitter whiff avg only | unavailable | unavailable | No | teamId | Unknown | Unknown | Unknown | Leave null. |
| Opponent away whiff% | No | Same as above | none | unavailable | unavailable | No | teamId | Unknown | Unknown | Unknown | Leave null. |
| Opponent vs-LHP K% | Stubbed null, likely feasible | MLB StatsAPI team hitting split if verified; current code has fields | `kRateVsLhp` exists in `generate-mlb-k-workload-shadow.mjs` but is currently set null | team strikeouts vs LHP | PA vs LHP | Planned fields include `plateAppearancesVsLhp` | teamId, season, pitcher hand split | Additional per-team split calls or expanded stat call | Medium | None | Highest-value next enrichment; implement only after endpoint contract is fixture-tested. |
| Opponent vs-RHP K% | Stubbed null, likely feasible | Same as above | `kRateVsRhp` exists but is null | team strikeouts vs RHP | PA vs RHP | Planned fields include `plateAppearancesVsRhp` | teamId, season, pitcher hand split | Medium | Medium | None | Same as vs-LHP. |

Current recommendation: keep all missing venue/handedness split fields null in V2 inputs until the workload generator can populate them from verified MLB StatsAPI responses with explicit PA/BF sample counts. The only split enrichment that appears immediately worth pursuing inside the existing provider set is opponent K% vs LHP/RHP, because the current team-adjustment code already has nullable fields and confidence flags for it.
