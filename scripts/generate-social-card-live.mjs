 ...
 *
 * Usage:
 *   node scripts/generate-social-card-live.mjs --edition=morning [--slate-date=YYYY-MM-DD] [--raw=...] [--best-bets=...] [--output-dir=...]
...
async function main() {
  ...
  const { raw, bestBets } = resolveMlbDailyCardSource({ edition, slateDate, rawPath, bestBetsPath });

  if (edition === 'morning') {
    const { data, diagnostics } = buildMlbDailyMorningCardInput({ raw, bestBets, slateDate });
