 ...
 * publisher for K. This script is retained for forensic/manual dry-run use
 * only -- its scheduled invocation (poll-mlb-x-posts.yml's post-k job) is
 ...
 * (.cache/mlb-strikeout-props-x-posted), structurally invisible to the
 * canonical git-backed receipt, so a deliberate live run here is NOT
 * protected against duplicating a canonical post for the same slate date.
 */
