...
export function parseResolverArgs(argv: string[]): ResolverCliArgs {
  ...
  for (const raw of argv) {
    if (raw === "--dry-run") args.dryRun = true;
      ...
      const supported = new Set<PredictionType>(["spread", "passing", "rushing", "receiving", "team_opportunity", "team_total"]);
      if (!types.length || types.some((type) => !supported.has(type))) throw new Error("--prediction-types must contain spread, passing, rushing, receiving, team_opportunity, and/or team_total");
