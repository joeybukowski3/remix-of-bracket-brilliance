 ...
 *   --mode=repricing (WU6.8): STAGE B ONLY. Football context/evidence has
 *     NOT changed since the last handicap run (the WU6 planner only ever
 *     schedules this mode when that holds) -- only the sportsbook market
 *     has moved or first became available. Reconstructs the exact locked
 *     Stage A output from the latest WU4.6-compatible snapshot
 *     (reconstructLockedStageAFromSnapshot) and calls the SAME
 *     runGrokStageBInitial used by --mode=initial's Stage B, so there is no
 *     forked prompt/validation path. combineRepricingStage() carries
 *     thesis/matchupFactors/failureModes/evidenceQualityAssessment/
 *     independentPrediction/blindPrediction forward byte-for-byte -- it has
 *     no parameter through which a revised football prediction could even
 *     enter. updateAssessment is null (a market-only repricing is not a
 *     football-development assessment).
 *
...
async function main(): Promise<void> {
  ...
  if (args.mode === "initial") {
    ...
    const stageBResult = await runGrokStageBInitial({ game, lockedStageA, currentMarketState, apiKey });
    if (!stageBResult.ok) {
      console.error(`Grok Stage B repricing FAILED: ${stageBResult.error}`);
      if (stageBResult.telemetry) console.error("Telemetry at failure:", JSON.stringify(stageBResult.telemetry, null, 2));
      process.exitCode = 1;
      return;
    }
