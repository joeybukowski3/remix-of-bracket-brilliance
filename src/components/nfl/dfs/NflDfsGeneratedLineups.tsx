...
function LineupCard({ lineup }: { lineup: GeneratedLineup }) {
    ...
    <div role="tabpanel" aria-label={lineup.strategyLabel} className="space-y-3">
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white">
        <table className="w-full min-w-[36rem] border-collapse text-left text-[11px]">
          <thead>
            <tr className="uppercase tracking-wide text-slate-500">
              <th scope="col" className="px-3 py-2 font-black">Slot</th>
              <th scope="col" className="px-3 py-2 font-black">Player</th>
              <th scope="col" className="px-3 py-2 font-black">Matchup</th>
              <th scope="col" className="px-3 py-2 text-right font-black">Salary</th>
              <th scope="col" className="px-3 py-2 text-right font-black">JKB Proj</th>
              <th scope="col" className="px-3 py-2 text-right font-black">DK Avg PPG</th>
              <th scope="col" className="px-3 py-2 text-right font-black">Score</th>
            </tr>
          </thead>
          <tbody>
            {lineup.slots.map((slot) => (
              <Fragment key={slot.dkId}>
                <tr className="border-t border-slate-100">
                  <td className="px-3 py-2 font-black text-slate-500">{slot.slot}</td>
                  <td className="px-3 py-2">
                    <button
                      type="button"
                      onClick={() => setExpanded((current) => (current === slot.dkId ? null : slot.dkId))}
                      aria-expanded={expanded === slot.dkId}
                      className="text-left font-bold text-slate-900 underline decoration-dotted underline-offset-2"
                    >
                      {slot.playerName}
                    </button>
                    <span className="block text-[10px] text-slate-500">
                      {slot.team.toUpperCase()} {slot.position}
                      {slot.roleContext ? " · " + slot.roleContext.roleClass : ""}
                    </span>
                  </td>
                  <td className="px-3 py-2 text-slate-600">{slot.opponent ? "vs " + slot.opponent.toUpperCase() : "N/A"}</td>
                  <td className="px-3 py-2 text-right font-bold tabular-nums text-slate-900">{money(slot.salary)}</td>
                  <td className="px-3 py-2 text-right tabular-nums text-slate-900">
                    {slot.projectedFantasyPoints == null ? <span className="text-slate-400">no JKB proj</span> : num(slot.projectedFantasyPoints, 2)}
                  </td>
                  <td className="px-3 py-2 text-right tabular-nums text-slate-600">{num(slot.dkAvgPointsPerGame, 1)}</td>
                  <td className="px-3 py-2 text-right font-bold tabular-nums text-slate-900">{num(slot.strategyScore.score, 1)}</td>
                </tr>
                {expanded === slot.dkId && (
                  <tr>
                    <td colSpan={7} className="p-0">
                      <PlayerReasoning slot={slot} />
                    </td>
                  </tr>
                )}
              </Fragment>
...
export default function NflDfsGeneratedLineups({ analysis, projectionRows, asOf }: NflDfsGeneratedLineupsProps) {
  const [active, setActive] = useState<LineupStrategy>("ceiling");
  const optimizer = useNflDfsOptimizer({ rows: analysis?.rows ?? null, projectionRows, asOf });
    ...
    <section aria-labelledby="dfs-generated-lineups" className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
      ...
      {result && (
        <>
          ...
          {lineup && <LineupCard lineup={lineup} />}
          ...
          <p className="text-[10px] text-slate-500">
            ...
            {result.candidatePool.uploadedRows} uploaded rows. Every uploaded player remains visible on the board above.
            Generated in {result.elapsedMs} ms.
          </p>
        </>
      )}
