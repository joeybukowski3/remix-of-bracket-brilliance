...
import { METRIC_NA, type ComparisonSideValue } from "@/lib/nfl/matchupMetrics";
...
function ComparisonSide({
  ...
  return (
    <div
      className={`flex flex-col gap-0.5 rounded px-1 py-0.5 ${rankCellClass(value.rank)} ${
    ...
    >
      ...
      <MatchupRankBadge rank={value.rank} />
    </div>
  );
}
...
export default function MatchupComparisonRow({
  ...
  return (
    <div className="grid grid-cols-[4.25rem_minmax(0,1fr)_4.25rem] items-center gap-1.5 border-b border-slate-100 py-1.5 last:border-0 sm:grid-cols-[6.5rem_minmax(0,1fr)_6.5rem] sm:gap-2">
      <ComparisonSide side="away" value={away} teamName={awayTeamName} metricLabel={metricLabel} />
      ...
      <ComparisonSide side="home" value={home} teamName={homeTeamName} metricLabel={metricLabel} />
    </div>
  );
}
