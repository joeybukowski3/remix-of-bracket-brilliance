...
import { buildOpponentLast10Summary } from "@/lib/nfl/props/review/yardageHistoryView";
import {
        ...
        <table className="w-full min-w-[760px] border-collapse text-[11px]">
          <thead>
            <tr className="border-b-2 border-slate-300 bg-slate-200/70 text-left text-[10px] font-bold uppercase tracking-wide text-slate-600">
              ...
              <th className="px-2 py-1.5 bg-slate-100/80">{MARKET_YPG_LABEL[market]}</th>
              <th className="px-2 py-1.5 bg-slate-100/80">{MARKET_ALLOWED_LABEL[market]}</th>
          ...
          <tbody>
            {history.games.map((game) => (
              <tr key={`${game.gameId ?? `${game.season}-${game.week}`}-${game.opponentPlayerId}`} className="border-b border-slate-100 last:border-b-0">
                ...
                <td className="px-2 py-1.5"><NflYardageRankCell rank={game.oppOffRank} /></td>
                <td className="px-2 py-1.5 bg-slate-50/70 tabular-nums text-slate-700">{fmt1(game.oppPlayerYpg)}</td>
