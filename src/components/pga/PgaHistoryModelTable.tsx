    ...
    <>
        ...
        <table className="w-full min-w-[1180px] table-fixed text-center text-[12px] leading-tight">
          ...
          <thead>
            <tr className="bg-slate-900 text-[11px] font-black uppercase tracking-[0.08em] text-white">
              <th rowSpan={2} className="px-1 py-2.5" title="Field Rank is the position among this week's entrants. Tour Rank, shown beneath it, is the position among every player with statistics.">Rank</th>
            ...
            <tr className="border-b border-slate-200 bg-slate-50 text-[10px] font-black uppercase text-slate-600">
              ...
              {isMajor ? <><th className="border-l border-slate-200 px-1 py-2 text-slate-500">Latest 4</th><th className="border-l border-slate-200 px-1 py-2 text-slate-500">Latest 8</th></> : <th className="border-l border-slate-200 px-1 py-2 text-slate-500">Latest 4</th>}
            </tr>
          </thead>
      ...
      <div className="space-y-2 lg:hidden">
        {rows.map((row) => {
          ...
          return <MobileCard key={row.player} row={row} statView={statView} isMajor={isMajor} eventLabel={eventLabel} rankMode={rankMode} scorePercentile={scorePercentiles.get(row.modelScore) ?? null} trendRanking={rankingMap.get(normalizePlayerKey(row.player)) ?? null} expanded={isExpanded} onToggle={() => setExpandedPlayer(isExpanded ? null : row.player)} />;
        })}
      </div>
    </>
  );
...
function DesktopColumnWidths({ isMajor }: { isMajor: boolean }) {
  return <colgroup><col style={{ width: "34px" }} /><col data-testid="pga-player-column" style={{ width: "210px", minWidth: "210px" }} /><col style={{ width: "64px" }} />{statKeys.map((key) => <col key={key} style={{ width: "58px" }} />)}<col style={{ width: "58px" }} /><col style={{ width: "74px" }} /><col style={{ width: isMajor ? "220px" : "240px" }} /><col style={{ width: isMajor ? "150px" : "250px" }} />{isMajor ? <col style={{ width: "200px" }} /> : null}</colgroup>;
}
...
function DesktopRow({ row, index, statView, isMajor, rankMode, scorePercentile, trendRanking }: { row: PgaTournamentModelRow; index: number; statView: "percentile" | "raw"; isMajor: boolean; rankMode: PgaRankMode; scorePercentile: number | null; trendRanking: JkbTrendRanking | null; }) {
  ...
  return <tr className={`${bg} hover:bg-emerald-50/40`}>
    ...
    {isMajor ? <><td className="border-b border-l border-slate-100 px-1 py-1.5"><FinishStrip values={row.specificMajorResults} count={4} /></td><td className="border-b border-l border-slate-100 px-1 py-1.5"><FinishStrip values={row.allMajorResults} count={8} dense /></td></> : <td className="border-b border-l border-slate-100 px-1 py-1.5"><FinishStrip values={row.eventResults} count={4} /></td>}
  </tr>;
}

function MobileCard({ row, statView, isMajor, eventLabel, rankMode, scorePercentile, trendRanking, expanded, onToggle }: { row: PgaTournamentModelRow; statView: "percentile" | "raw"; isMajor: boolean; eventLabel: string; rankMode: PgaRankMode; scorePercentile: number | null; trendRanking: JkbTrendRanking | null; expanded: boolean; onToggle: () => void; }) {
  const panelId = `pga-player-details-${normalizePlayerKey(row.player)}`;
  const rankDisplay = resolveRankDisplay(row, rankMode);
  return <article className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><button type="button" className="flex w-full min-w-0 items-center gap-2 px-3 py-3 text-left" aria-expanded={expanded} aria-controls={panelId} onClick={onToggle}><span className="w-8 shrink-0 text-[11px] font-black tabular-nums text-slate-500">#{rankDisplay.primaryValue}</span><span className="min-w-0 flex-1"><PlayerName player={row.player} mobile /></span><Score value={row.modelScore} percentile={scorePercentile} /><span aria-hidden="true" className="w-4 shrink-0 text-center text-sm font-black text-slate-500">{expanded ? "▲" : "▼"}</span><span className="sr-only">{expanded ? "Hide details" : "View details"}</span></button>
    {expanded ? <div id={panelId} className="border-t border-slate-100 px-3 pb-3"><p className="pt-2 text-[11px] text-slate-500">Tour rank <span className="font-bold text-slate-700">#{row.modelRank}</span> of all players with statistics{row.fieldRank != null ? <>{" · "}Field rank <span className="font-bold text-slate-700">#{row.fieldRank}</span> of this week&apos;s entrants</> : <>{" · "}Not in this week&apos;s field</>}</p><MobileSection title="Player Stats"><div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">{statKeys.map((key, index) => statView === "percentile" ? <PercentileMetric key={key} label={statLabels[index]} value={row.displayPercentiles[key] ?? null} /> : <Metric key={key} label={statLabels[index]} value={raw(row, key)} />)}</div></MobileSection><MobileSection title="Model"><div className="grid grid-cols-2 gap-2"><PercentileMetric label="Course Fit" value={row.courseFit} plainValue /><Metric label="JKB Trend" value={trendMetric(trendRanking, row.trend.label)} /></div></MobileSection><MobileSection title="Last 5 Starts"><FinishStrip values={row.recentResults} count={RECENT_START_COUNT} trendStyle /></MobileSection><MobileSection title="Tournament History"><div className="space-y-3">{isMajor ? <><Strip label="Specific major" values={row.specificMajorResults} count={4} /><Strip label="Last 8 majors" values={row.allMajorResults} count={8} /></> : <Strip label={`${eventLabel} history`} values={row.eventResults} count={4} />}</div></MobileSection></div> : null}
  </article>;
}

function MobileSection({ title, children }: { title: string; children: ReactNode }) { return <section className="mt-3"><h3 className="mb-1.5 text-[10px] font-black uppercase tracking-wide text-slate-500">{title}</h3>{children}</section>; }
function Group({ count, children }: { count: number; children: ReactNode }) { return <th colSpan={count} className="border-l border-slate-700 px-1 py-2 text-emerald-300">{children}</th>; }
function FinishStrip({ values, count, trendStyle = false, dense = false }: { values: PgaHistoryResult[]; count: number; trendStyle?: boolean; dense?: boolean; }) { return <div className={`grid w-full items-center gap-0.5 ${gridClass(count)}`}>{Array.from({ length: count }, (_, index) => <Finish key={index} value={values[index]} trendStyle={trendStyle} dense={dense} />)}</div>; }
function Finish({ value, trendStyle = false, dense = false }: { value?: PgaHistoryResult; trendStyle?: boolean; dense?: boolean; }) {
  if (!value) return <div className="flex h-7 min-w-0 items-center justify-center text-[11px] text-slate-300">—</div>;
  const position = value.finishPosition ?? 999; const isWin = value.madeCut && position === 1; const title = [value.eventName, value.season, value.finishText].filter(Boolean).join(" · ");
  const badgeClass = isWin ? "border-amber-500 bg-gradient-to-b from-amber-300 to-amber-400 text-amber-950 shadow-sm" : !value.madeCut ? value.status === "missed_cut" ? "border-slate-300 bg-slate-100 text-slate-700" : "border-rose-300 bg-rose-100 text-rose-900" : position <= 5 ? "border-emerald-400 bg-emerald-200 text-emerald-950" : position <= 10 ? "border-emerald-300 bg-emerald-100 text-emerald-900" : position <= 20 ? "border-cyan-200 bg-cyan-50 text-cyan-900" : position <= 40 ? "border-slate-200 bg-white text-slate-700" : "border-slate-200 bg-slate-50 text-slate-600";
  const dotClass = isWin ? "bg-amber-400" : !value.madeCut ? "bg-red-400" : position <= 10 ? "bg-emerald-500" : position <= 25 ? "bg-amber-300" : "bg-slate-300";
  return <div title={title} className="flex min-w-0 flex-col items-center justify-center gap-0.5">{trendStyle ? <span className={`h-1 w-1 rounded-full ${dotClass}`} /> : null}<span className={`flex h-6 w-full min-w-0 items-center justify-center rounded border px-0.5 font-black tabular-nums ${dense ? "text-[9px]" : "text-[11px]"} ${badgeClass}`}>{isWin ? "1" : value.finishText}</span></div>;
}
...
function Strip({ label, values, count, trendStyle = false }: { label: string; values: PgaHistoryResult[]; count: number; trendStyle?: boolean; }) { return <div className="mt-3 first:mt-0"><div className="mb-1 text-[9px] font-black uppercase text-slate-400">{label}</div><FinishStrip values={values} count={count} trendStyle={trendStyle} dense={count === 8} /></div>; }
function gridClass(count: number) { if (count === 8) return "grid-cols-8"; if (count === 5) return "grid-cols-5"; return "grid-cols-4"; }
function compactStatLabel(label: string) { if (label === "Drive Acc.") return <>Drv<br />Acc</>; if (label === "Drive Dist.") return <>Drv<br />Dist</>; return label.replace("SG ", ""); }
...
function signed(value: number | null) { if (value == null || !Number.isFinite(value)) return "—"; return `${value > 0 ? "+" : ""}${value.toFixed(2)}`; }
function trendMetric(ranking: JkbTrendRanking | null, fallback: string) { if (ranking?.rank == null) return fallback; return `#${ranking.rank}${ranking.confidence === "provisional" ? "*" : ""} · ${signed(ranking.recent20)}`; }
function availableTrendTours(sources: Record<string, { status?: string }> | undefined) { const available = Object.entries(sources ?? {}).filter(([, source]) => source.status === "available").map(([tour]) => tour); return available.length ? available.join("/") : "available tour"; }
