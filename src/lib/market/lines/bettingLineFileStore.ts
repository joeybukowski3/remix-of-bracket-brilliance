...
/**
 * WU8 file-backed persistence for betting lines. It reuses only the generic FS
 * helpers from {@link ./bettingLineFsUtils}.
 *
...
export function createBettingLineFileStore(
  ...
  return {
    ...
    async extendSnapshotObservation(id, observation) {
      ...
      if (filePath === null) {
        throw new BettingLineFileStoreError(`Unknown betting-line snapshot id: ${id}`);
      }
      const rows = await readGameRows(filePath);
