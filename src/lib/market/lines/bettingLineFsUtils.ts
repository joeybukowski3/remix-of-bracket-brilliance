...
/**
 * Filesystem helpers for the WU8 file-backed betting-lines store and the public
 * artifact publisher.
 *
...
export class BettingLineFileStoreError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "BettingLineFileStoreError";
  }
}
 ...
 * already safe (`2026_01_NE_SEA`); this only guards against exotic provider ids.
 */
export function toGameFileToken(jkbGameId: string): string {
  const cleaned = jkbGameId.replace(/[^A-Za-z0-9_-]/g, "_");
  if (cleaned.length === 0) {
    throw new BettingLineFileStoreError(
      `Cannot route a betting-lines history file for jkbGameId ${JSON.stringify(jkbGameId)}.`,
    );
  }
  return cleaned;
}
