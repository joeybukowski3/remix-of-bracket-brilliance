...
describe("createTheOddsApiClient", () => {
  ...
  it("builds the CFB endpoint from the ncaaf sport key", async () => {
    const fetchImpl = vi.fn(async () => response([]));
    const client = createTheOddsApiClient({
      apiKey: KEY,
      fetchImpl: fetchImpl as unknown as typeof fetch,
    });
    await client.getCurrentOdds({ league: "cfb" });
    const url = new URL(fetchImpl.mock.calls[0][0] as string);
    expect(url.pathname).toBe("/v4/sports/americanfootball_ncaaf/odds");
  });
