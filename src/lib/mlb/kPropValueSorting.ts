 ...
 */
export function sortByAbsoluteProjectionEdge<T extends Pick<PitcherStrikeoutTeamRow, "projectedKs" | "kLine">>(rows: T[]): T[] {
  return [...rows].sort((a, b) => {
    ...
    return (right.absoluteProjectionEdge as number) - (left.absoluteProjectionEdge as number);
  });
}
