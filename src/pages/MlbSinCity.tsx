  ...
  return (
    <div className={`rounded-lg border px-3 py-2 ${unavailable ? "border-slate-200 bg-slate-50" : criterion.pass ? "border-emerald-200 bg-emerald-50" : "border-rose-200 bg-rose-50"}`}>
      ...
      <div className="mt-1 text-sm font-black text-slate-900">
        {isWind ? criterion.detail ?? "Unavailable" : criterion.name === "Exit Velo" ? formatMetric(criterion.value, " mph") : formatMetric(criterion.value)}
      </div>
      <div className="text-[10px] text-slate-500">{isWind ? `Pass: blowing out at ${criterion.threshold}+ mph` : `Threshold: ${criterion.threshold}${criterion.name === "Exit Velo" ? " mph" : "%"}`}</div>
    </div>
  );
}
    ...
    <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <MlbTeamLogo team={batter.team} className="h-11 w-11 shrink-0" />
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">{evaluation.criteria.map((criterion) => <CriterionPill key={criterion.name} criterion={criterion} />)}</div>
...
export default function MlbSinCity() {
  usePageSeo({ title: "Sin City MLB Home Run Model | Joe Knows Ball", description: "A separate MLB home-run candidate model using four batter-power thresholds plus favorable wind.", path: "/mlb/sin-city" });
  ...
  const enrichedBatters: SinCityBatter[] = batters.map((batter) => {
    ...
    return { ...batter, stadium: game?.stadium ?? batter.ballpark, roofType: game?.roofType ?? "Unknown", windDirection: game?.windDirection ?? "", windSpeed: game?.windSpeed ?? null };
  });
      ...
      <main className="min-h-screen bg-slate-50 py-6">
        <div className="mx-auto max-w-7xl space-y-6 px-4 sm:px-6">
          ...
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-5">
            {[["5 of 5", fiveOfFive], ["4 of 5", fourOfFive], ["3 of 5", threeOfFive], ["Players Evaluated", enrichedBatters.length], ["Mode", result.isFallback ? "Closest 5" : "Qualifiers"]].map(([label, value]) => (
