...
import { contrarianSides, formatSplitsGap, formatSplitsLine, formatSplitsOdds, publicSides, sharpSides, sortSplitsMatchupRows, sortSplitsRows, splitsMatchupRows, splitsRows, splitsSignal, SPLITS_SIGNAL_CLASS, SPLITS_SIGNAL_LABEL, type SplitsFavorites, type SplitsMarket, type SplitsMatchupRow, type SplitsMatchupSortKey, type SplitsRow, type SplitsSortKey } from "@/lib/nfl/bettingSplitsView";
...
function Favorite({ row, side, metric, logo = true }: { row: SplitsMatchupRow; side: SplitsFavorites["handle"]; metric: "handlePct" | "betsPct"; logo?: boolean }) {
  ...
  return <span className="inline-flex items-center gap-1 whitespace-nowrap font-semibold tabular-nums text-slate-900">{logo && (side.side === "away" || side.side === "home") && <TeamLogo name={label.toUpperCase()} logo={nflLogoUrl(label)} fallbackLabel={label.slice(0, 2).toUpperCase()} className="h-4 w-4 shrink-0 bg-transparent" />}<span>{side.side === "away" || side.side === "home" ? label.toUpperCase() : label} {side[metric]}%</span></span>;
}

function MobileFavorites({ row, title, favorites }: { row: SplitsMatchupRow; title: string; favorites: SplitsFavorites }) {
  return <div className="grid grid-cols-[4.5rem_minmax(0,1fr)_minmax(0,1fr)] items-center gap-2 border-t border-slate-100 py-1.5 text-[11px] first:border-0"><strong className="text-slate-700">{title}</strong><span className="min-w-0"><span className="block text-[10px] text-slate-500">Handle</span><Favorite row={row} side={favorites.handle} metric="handlePct" /></span><span className="min-w-0"><span className="block text-[10px] text-slate-500">Bets</span><Favorite row={row} side={favorites.bets} metric="betsPct" /></span></div>;
}
...
function OverviewTable({ rows }: { rows: readonly SplitsMatchupRow[] }) {
  ...
  return <section className="min-w-0 overflow-hidden rounded-lg border border-slate-200 bg-white">
    ...
    <div role="region" aria-label="Overview matchup cards" className="divide-y divide-slate-200 md:hidden">{shown.map((row) => <article key={row.game.gameId} data-splits-game-id={row.game.gameId} className="px-3 py-2.5"><div className="flex flex-wrap items-center justify-between gap-1.5"><MatchupMark away={row.game.away} home={row.game.home} /><SignalBadge gap={row.strongest.gap} /></div><div className="mt-1.5"><MobileFavorites row={row} title="Spread" favorites={row.spread} /><MobileFavorites row={row} title="Total" favorites={row.total} /><MobileFavorites row={row} title="Moneyline" favorites={row.moneyline} /></div><div className="flex flex-wrap items-center justify-between gap-1 border-t border-slate-100 pt-1.5 text-[11px]"><span className="text-slate-600">Top Gap · {strongestLabel(row.strongest)}</span><GapCell gap={row.strongest.gap} /></div></article>)}</div>
    <DenseTableScroller label="Overview matchup distribution" className="hidden w-full md:block"><table className="min-w-[820px] w-full text-[11px]"><thead><tr className={DENSE_TABLE_HEAD_ROW}><th rowSpan={2} scope="col" className="px-2 py-2 text-left">{sortButton("matchup", "Matchup")}</th>{["Spread", "Total", "Moneyline"].map((market) => <th key={market} colSpan={2} scope="colgroup" className="border-l border-slate-200 px-2 py-1.5 text-center">{market} betting splits</th>)}<th rowSpan={2} scope="col" className="border-l border-slate-200 px-2 py-2 text-right">{sortButton("gap", "Money Gap")}</th><th rowSpan={2} scope="col" className="px-2 py-2 text-left">Signal</th></tr><tr className={DENSE_TABLE_HEAD_ROW}>{["Spread", "Total", "Moneyline"].flatMap((market) => [<th key={`${market}-handle`} scope="col" className="border-l border-slate-200 px-2 py-1.5 text-left">Handle Favorite</th>, <th key={`${market}-bets`} scope="col" className="px-2 py-1.5 text-left">Bets Favorite</th>])}</tr></thead><tbody>{shown.map((row) => <tr key={row.game.gameId} data-splits-game-id={row.game.gameId} className={DENSE_TABLE_ROW}><td className="whitespace-nowrap px-2 py-2"><MatchupMark away={row.game.away} home={row.game.home} /></td>{[row.spread, row.total, row.moneyline].flatMap((market, index) => [<td key={`${index}-h`} className="border-l border-slate-200 px-2 py-2"><Favorite row={row} side={market.handle} metric="handlePct" /></td>, <td key={`${index}-b`} className="px-2 py-2"><Favorite row={row} side={market.bets} metric="betsPct" /></td>])}<td className="border-l border-slate-200 px-2 py-2 text-right whitespace-nowrap"><div className="text-[10px] text-slate-600">{strongestLabel(row.strongest)}</div><GapCell gap={row.strongest.gap} /></td><td className="px-2 py-2"><SignalBadge gap={row.strongest.gap} /></td></tr>)}</tbody></table></DenseTableScroller>
...
function RankedTable({ title, rows, tone }: { title: string; rows: readonly SplitsRow[]; tone: "public" | "sharp" | "contrarian" }) {
  ...
  return <section aria-label={title} className="min-w-0 overflow-hidden rounded-lg border border-slate-200 bg-white"><h2 className={cn("border-b px-3 py-2 text-xs font-bold", tones[tone])}>{title}</h2><div className="overflow-x-auto"><table className="w-full min-w-[360px] text-[11px]"><thead><tr className={DENSE_TABLE_HEAD_ROW}><th scope="col" className="px-3 py-1.5 text-left">Side</th><th scope="col" className="px-2 py-1.5 text-left">Market</th><th scope="col" className="px-2 py-1.5 text-right">Handle</th><th scope="col" className="px-2 py-1.5 text-right">Bets</th><th scope="col" className="px-3 py-1.5 text-right">Gap</th></tr></thead><tbody>{rows.slice(0, 5).map((row) => <tr key={`${row.game.gameId}:${row.market}:${row.side.side}`} className={DENSE_TABLE_ROW}><td className="whitespace-nowrap px-3 py-1.5">{row.side.side === "away" || row.side.side === "home" ? <TeamMark abbr={row.side.side === "away" ? row.game.away : row.game.home} /> : row.side.side === "over" ? "Over" : "Under"}</td><td className="px-2 py-1.5 capitalize">{row.market}</td><td className="px-2 py-1.5 text-right tabular-nums">{row.side.handlePct}%</td><td className="px-2 py-1.5 text-right tabular-nums">{row.side.betsPct}%</td><td className="px-3 py-1.5 text-right tabular-nums"><GapCell gap={row.gap} /></td></tr>)}</tbody></table>{rows.length === 0 && <p className="px-3 py-3 text-xs text-slate-600">No sides meet this definition this week.</p>}</div></section>;
}
...
function MobileMarketRow({ row, market }: { row: SplitsRow; market: SplitsMarket }) {
  ...
  const matchup = row.side.side === "away" ? `${row.game.away.toUpperCase()} @ ${row.game.home.toUpperCase()}` : row.side.side === "home" ? `${row.game.home.toUpperCase()} vs ${row.game.away.toUpperCase()}` : `${row.game.away.toUpperCase()} @ ${row.game.home.toUpperCase()} · ${row.side.side === "over" ? "Over" : "Under"}`;
  ...
  return <article data-splits-game-id={row.game.gameId} className="px-3 py-2.5 text-xs"><div className="flex flex-wrap items-center justify-between gap-1"><strong className="text-slate-900">{matchup}</strong><SignalBadge gap={row.gap} /></div><div className="mt-1 flex flex-wrap items-center justify-between gap-2 text-[11px] tabular-nums"><span className="inline-flex items-center gap-1">{logo}<span>Handle <strong>{row.side.handlePct}%</strong></span></span><span className="inline-flex items-center gap-1">{logo}<span>Bets <strong>{row.side.betsPct}%</strong></span></span><span className="text-slate-600">{market !== "moneyline" && <>{formatSplitsLine(market, row.side.line)} · </>}{formatSplitsOdds(row.side.odds)}</span></div><div className="mt-1 text-[11px] text-slate-600">Money Gap <GapCell gap={row.gap} /></div></article>;
}
