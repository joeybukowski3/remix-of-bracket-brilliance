...
export default function NFLCoachOfYear2026() {
  ...
  return <>
    <NflPageHeader eyebrow="NFL awards research" title="2026 Coach of the Year candidate model" description="Historical winner traits, a transparent elimination funnel and a tiered 2026 candidate board built around playoff viability, model improvement and market expectations." />
    <NflSection eyebrow="Historical profile" title="What the last 10 winners had in common" subtitle="Award seasons 2016–2025. Team logos and heat-shaded cells make the common winner traits easier to scan." collapse="mobile"><NflMetricStrip ariaLabel="Historical winner profile" columns={5} metrics={[{ label: "First-year coach", value: `${summary.firstYearCoachPct}%` }, { label: "Missed prior playoffs", value: `${summary.missedPriorPlayoffsPct}%` }, { label: "Award-year playoffs", value: `${summary.awardPlayoffsPct}%`, tone: "good" }, { label: "Won division", value: `${summary.divisionWinnerPct}%` }, { label: "Improved scoring", value: `${summary.improvedPpgPct}%` }, { label: "Avg win increase", value: `+${summary.averageWinIncrease.toFixed(1)}`, tone: "good" }, { label: "Avg PPG increase", value: `+${summary.averagePpgIncrease.toFixed(1)}`, tone: "good" }, { label: "Easier award SOS", value: `${summary.easierSchedulePct}%` }, { label: "Avg prior SOS", value: summary.averagePriorSos.toFixed(3) }, { label: "Avg award SOS", value: summary.averageAwardSos.toFixed(3) }]} /><HistoryTable /></NflSection>
    <NflSection eyebrow="2026 candidate funnel" title="Eliminate, downgrade, then tier the remaining field" subtitle="Prior playoff teams are removed, winning non-playoff teams and verified major schedule jumps are downgraded, and the remaining candidates are grouped by award viability."><NflMetricStrip ariaLabel="Candidate funnel" columns={3} metrics={[{ label: "Eliminated", value: String(counts.eliminated), detail: "Made the 2025 playoffs", tone: "bad" }, { label: "Unlikely", value: String(counts.unlikely), detail: "Winning record or major SOS jump" }, { label: "Remaining field", value: String(counts.rated), detail: "Advanced to the tiered board", tone: "good", primary: true }]} /></NflSection>
    <NflSection eyebrow="Top four profiles" title="The case for and against the leading candidates" subtitle="Each team has a full-width expandable profile. New Orleans is open by default."><TopCandidateAccordions rows={topProfiles} /><p className="mt-3 text-[10px] leading-5 text-slate-500">Coach of the Year odds are consensus or featured prices published by SportsBettingDime on June 9, 2026, with Todd Monken updated to +2200.</p></NflSection>
    <NflSection eyebrow="Tiered candidate board" title="Remaining 2026 Coach of the Year candidates" subtitle="Vegas O/U, Coach of the Year odds, unit ratings, coach tenure and model projection remain visible. First-year coaches are labelled as first-year." bodyClassName="!px-0"><TieredCandidateTable candidateByAbbr={candidateByAbbr} /></NflSection>
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
      <NflSection eyebrow="Secondary cut" title="Unlikely profiles" subtitle="Missed the playoffs, but begin with a less typical winner profile because they already had a winning record or face a verified major schedule increase." collapse="mobile" defaultOpen={false} bodyClassName="!px-0"><CandidateTable rows={COACH_OF_YEAR_UNLIKELY} /></NflSection>
      <NflSection eyebrow="Automatic cut" title="Eliminated: 2025 playoff teams" subtitle="Prior playoff teams are removed in this first version of the model." collapse="mobile" defaultOpen={false} bodyClassName="!px-0"><CandidateTable rows={COACH_OF_YEAR_ELIMINATED} /></NflSection>
    </div>
  </>;
}

function HistoryTable() { return <div className="mt-4 overflow-hidden rounded-lg border border-slate-200"><div className="overflow-x-auto" role="region" aria-label="Past Coach of the Year winners" tabIndex={0}><table className="w-full min-w-[1220px] text-xs"><thead><tr className="bg-slate-100 text-[9px] font-semibold uppercase tracking-wider text-slate-600"><th className="px-3 py-3 text-left">Season</th><th className="px-3 py-3 text-left">Coach / Team</th><th>Years with team</th><th>Prior record</th><th>Prior playoffs</th><th>Award record</th><th>Win increase</th><th>Division</th><th>PPG increase</th><th>Award playoffs</th><th>Prior SOS</th><th>Award SOS</th></tr></thead><tbody>{COACH_OF_YEAR_HISTORY.map((row) => { const easier = row.awardSos < row.priorSos; return <tr key={row.season} className="border-t border-slate-100 hover:bg-slate-50"><td className="px-3 py-3 font-black">{row.season}</td><td className="px-3 py-3"><div className="flex items-center gap-3"><img src={nflLogoUrl(row.teamAbbr)} alt="" className="h-9 w-9 object-contain" /><div><div className="font-black">{row.coach}</div><div className="text-slate-500">{row.team}</div></div></div></td><td className="text-center"><TenureBadge years={row.tenureYear} /></td><td className="text-center font-bold">{row.priorRecord}</td><BooleanCell value={row.priorPlayoffs} /><td className="bg-blue-50 text-center font-black text-blue-800">{row.awardRecord}</td><HistoricalHeatCell value={`+${row.winIncrease}`} strength={row.winIncrease / 10} positive /><BooleanCell value={row.divisionWinner} /><HistoricalHeatCell value={signed(row.ppgIncrease)} strength={row.ppgIncrease / 12} positive={row.ppgIncrease >= 0} /><BooleanCell value={row.awardPlayoffs} /><td className="text-center">{row.priorSos.toFixed(3)}</td><td className={`text-center font-black ${easier ? "bg-emerald-100 text-emerald-800" : row.awardSos > row.priorSos ? "bg-red-100 text-red-700" : "bg-slate-100"}`}>{row.awardSos.toFixed(3)}<div className="text-[8px] uppercase tracking-wider">{easier ? "easier" : row.awardSos > row.priorSos ? "harder" : "same"}</div></td></tr>; })}</tbody></table></div></div>; }
...
function TieredCandidateTable({ candidateByAbbr }: { candidateByAbbr: Map<string, CoachCandidateRow> }) { return <div className="overflow-hidden border-y border-slate-200"><div className="overflow-x-auto" role="region" aria-label="Tiered Coach of the Year candidate board" tabIndex={0}><table className="w-full min-w-[1420px] text-xs"><CandidateHeader showScore /><tbody>{REMAINING_TIERS.map((tier) => { const rows = tier.teams.map((abbr) => candidateByAbbr.get(abbr)).filter((row): row is CoachCandidateRow => Boolean(row)); return [<TierRow key={`${tier.key}-heading`} label={tier.label} description={tier.description} tone={tier.tone} />, ...rows.map((row, index) => <CandidateRow key={row.team.abbr} row={row} showScore rankLabel={`${index + 1}`} />)]; })}</tbody></table></div></div>; }
function CandidateTable({ rows }: { rows: CoachCandidateRow[] }) { return <div className="overflow-hidden border-y border-slate-200"><div className="overflow-x-auto" role="region" aria-label="Coach of the Year candidates" tabIndex={0}><table className="w-full min-w-[1120px] text-xs"><CandidateHeader /><tbody>{rows.map((row) => <CandidateRow key={row.team.abbr} row={row} />)}</tbody></table></div></div>; }
