  ...
  return <SiteShell><main className="bg-slate-50">
    ...
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      <article className="mt-8 rounded-2xl border bg-white p-5 shadow-sm sm:p-8 lg:p-12">
        ...
        <section className={sectionClass}><h2 className={headingClass}>What the JoeKnowsBall model values this week</h2><p className={paragraphClass}>The heaviest weights fall on strokes gained total, around the green and approach, followed by off-the-tee play, while putting receives a deliberately lighter weight. In plain terms, the model is hunting for complete ball-strikers who can save par from bunkers and greenside rough.</p><p className={paragraphClass}>The second major signal is <Link className="font-bold text-emerald-700 underline" to="/pga">JKB Trend</Link>, which measures recent-round scoring among tracked players. See <Link className="font-bold text-emerald-700 underline" to="/pga">the current JoeKnowsBall PGA model</Link> for complete rankings, course fit, major history and recent finishes.</p></section>
      ...
      <div className="mt-8 flex flex-wrap gap-3"><Link to="/pga" className="rounded-full bg-emerald-600 px-5 py-2.5 text-sm font-black text-white">View PGA Model</Link><Link to="/pga" className="rounded-full bg-slate-200 px-5 py-2.5 text-sm font-black text-slate-800">Back to PGA</Link></div>
    </div>
  </main></SiteShell>;
}
