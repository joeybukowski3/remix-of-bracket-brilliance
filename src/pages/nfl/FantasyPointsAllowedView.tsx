...
import { buildFantasyAllowedTableRows, fantasyAllowedColumns, fantasyAllowedRankTone } from "@/lib/nfl/fantasyAllowed/presentation";
 ...
 * fantasy points allowed by position (QB/RB/WR/TE, with 2026 WR splits). Owns its
...
export default function FantasyPointsAllowedView({
  ...
  const columns = useMemo(() => fantasyAllowedColumns(source.artifact, sample), [source.artifact, sample]);
  const activeSort = columns.some((column) => column.key === sort.key) || sort.key === "team" || sort.key === "opponent"
    ? sort
  ...
  return (
    <div>
      ...
      <section className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
        <p className="text-[11px] text-slate-500">Wide/Slot WR splits are available for 2026; other samples use combined WR.</p>
